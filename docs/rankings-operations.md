...
- 用户总览的今日、本周、本月查询会在 D1 内按 `userId + board_key + 重置日` 去重，并限制到当前周期窗口，再交给 Worker 汇总，避免把整个赛季的明细搬进 Worker。
...
页面打开榜单视图时先读取 D1，不主动抓取 CDK，也不要求安装用户脚本。只有手动点击“立即刷新”、开启“每 3 小时刷新”或触发一次自动刷新时，才通过 bridge 请求 CDK；此时才检查用户脚本版本。旧脚本会显示更新入口，未安装脚本会提示安装，但都不会清空已有榜单。普通网络错误或服务端错误在 1 小时后最多自动重试 1 次；403、429、盾页、验证页和非 JSON 响应不自动重试并进入 3 小时冷却。周期或榜单类型切换只读取 D1，不重复请求 CDK。默认不自动上传，关闭上传时新快照只在当前页面显示；开启自动上传或点击“上传云端”后才写入 D1。请求失败时页面保留旧快照并显示错误。
