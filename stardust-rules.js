(function attachStardustRules(global) {
    ...
    function getBoostStatus(now = Date.now(), { enabled = true, durationDays = BOOST_DEFAULT_DURATION_DAYS } = {}) {
        ...
        if (!enabled) {
            return { state: 'disabled', day: 0, durationDays: normalizedDuration, startAt: BOOST_START_AT, endAt, active: false };
        }
        if (!Number.isFinite(timestamp) || timestamp < BOOST_START_AT) {
            return { state: 'upcoming', day: 0, durationDays: normalizedDuration, startAt: BOOST_START_AT, endAt, active: false };
        }
        if (timestamp >= endAt) {
            return { state: 'ended', day: normalizedDuration, durationDays: normalizedDuration, startAt: BOOST_START_AT, endAt, active: false };
        }
        return {
            state: 'active',
    ...
    function getDailyQuotaForSeasonDay(day, {
        ...
        const freePulls = vip ? ordinaryFreePulls + VIP_EXTRA_DAILY_FREE_PULLS : ordinaryFreePulls;
        const paidPulls = vip ? ordinaryPaidPulls + VIP_EXTRA_DAILY_PAID_PULLS : ordinaryPaidPulls;
        return { seasonDay, boosted, freePulls, paidPulls, totalPulls: freePulls + paidPulls, paidCost: paidPulls * 10 };
    }
